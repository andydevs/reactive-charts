...
export function labelAnchor(style: PieStyle, slice: Slice): LabelAnchor {
    ...
    let y = -Math.sin(mid) * style.sliceMaxRadius - style.labelHeight
    let x = (isAngleLeft(mid) ? -1 : 1) * (style.sliceMaxRadius + style.labelOffset)
    let textAnchor: 'end' | 'start' = isAngleLeft(mid) ? 'end' : 'start'
...
export function labelPath(style: PieStyle, slice: Slice): Partial<SVGAttributes<SVGLineElement>> {
    ...
    let [x1, y1] = angleRadius(mid, style.sliceMaxRadius)
    return { x1, y1, x2, y2 }
}
